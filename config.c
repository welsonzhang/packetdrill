...
struct option options[] = {
    ...
    { "mk_tun",         .has_arg = false, NULL, OPT_MK_TUN  },
    { "rm_tun",         .has_arg = false, NULL, OPT_RM_TUN }, 
...
void set_default_config(struct config *config)
{
    ...
    config->is_mk_tun = false;
    config->is_rm_tun = false;
}
			   ...
			   char *where)
{
	...
	case OPT_VERBOSE:
		...
		break;
    case OPT_MK_TUN:
        config->is_mk_tun = true;
        break;
    case OPT_RM_TUN:
        config->is_rm_tun = true;
        break;
	default:
		show_usage();
		exit(EXIT_FAILURE);
